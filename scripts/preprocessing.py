...
output_dir = "data\\optimized_chunks"
...
for item, next_item in zip(
    all_filtered_bookmarks_info, all_filtered_bookmarks_info[1:] + [None]
):
    if next_item:
        final_pages_division_info.append(
            {**item, "final_page": next_item["start_page"] - 1}
        )
    else:
        final_pages_division_info.append({**item, "final_page": final_page - 1})
