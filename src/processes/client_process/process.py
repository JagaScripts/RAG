...
content_summaries = "\n".join(
    [f"{source}: {content}" for source, content in summaries.items()]
)
...
class SourceModel(BaseModel):
    ...
    reason: str = Field(
        ...,
        description="Razones por las que eliges la seleccion",
    )
...
def get_rag_prompt(context, question):
    return [
        ...
        UserContent(parts=Part(text=f"Pregunta: {question}")),
    ]
...
async def get_answer(query):
    ...
    answer = await llm_google_genai.aio.models.generate_content(
        model="gemini-2.5-flash-lite", contents=rag_prompt
    )
    return {
        "answer": answer.text,
